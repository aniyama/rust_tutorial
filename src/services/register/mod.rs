use crate::models;
use crate::services;
use chrono::NaiveDate;
use std::io;
use std::str::FromStr;

pub fn run(path: &str) {
    println!("登録");
    let register_type = input_register_type();
    let name = input_name();
    let category_type = input_category_type(register_type);
    let price = input_price();
    let date = input_date();
    let category = models::Item::get_category(register_type, category_type);

    let item = models::Item::new(name, category, price, date);
    println!("{:?}", &item);

    let mut data = services::io::read_date_or_create_date(path);
    data.push(item);
    services::io::write_to_json(&data, path);
}

fn input_register_type() -> u8 {
    println!("種別を入力してください");
    let mut register_type = String::new();
    io::stdin().read_line(&mut register_type).expect("入力失敗");
    let register_type: u8 = register_type
        .trim()
        .parse()
        .expect("数値で入力してください");
    services::validate::InputValidator::validate_register_type(register_type);
    register_type
}

fn input_name() -> String {
    println!("品目名を入力してください");
    let mut name = String::new();
    io::stdin()
        .read_line(&mut name)
        .expect("品目名の入力に失敗しました");
    name.trim().to_string()
}

fn input_category_type(register_type: u8) -> u8 {
    println!("カテゴリーを入力してくださいv");
    if register_type == 0 {
        println!("(0: CU, 1: FI, 2:)");
    } else {
        println!("(0: , 1: , 2)");
    }
    let mut category_type = String::new();
    io::stdin()
        .read_line(&mut category_type)
        .expect("カテゴリ名の入力に失敗しました");
    let categpry_type = category_type
        .trim()
        .parse()
        .expect("カテゴリは数値で入力してください");
    let category_type: u8 = category_type
        .trim()
        .parse()
        .expect("カテゴリは数値で入力してください");
    services::validate::InputValidator::validate_category_type(register_type, category_type);
    categpry_type
}

fn input_price() -> u32 {
    println!("金額を入力してください");
    let mut price = String::new();
    io::stdin()
        .read_line(&mut price)
        .expect("金額の入力に失敗しました");
    price
        .trim()
        .parse()
        .expect("金額は数値でにゅりょくしてください")
}

fn input_date() -> NaiveDate {
    println!("日付を入力してください");
    let mut date = String::new();
    io::stdin().read_line(&mut date).unwrap();
    return NaiveDate::from_str(&date).expect("日付はYYYY-mm-ddでにゅりょくしてください");
}
